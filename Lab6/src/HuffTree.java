...
public class HuffTree<Key, E> {
	...
	private void getCode(BinNode focus, String path) {
		//base case: current node has no children
		//general:  getCode(left, path + "0") getCode(right, path + "1")
		//codeTable.put(key, value)
		if (focus.getLeft() == null){codeTable.put((String) focus.getValue(), path);}
		if (focus.getRight() == null){codeTable.put((String) focus.getValue(), path);}
		if (focus.getLeft() != null){getCode(focus.getLeft(), path + "0");}
		if (focus.getRight() != null){getCode(focus.getRight(), path + "1");}
	}
