...
public class BSTJr <K extends Comparable<?super K>> {
	...
	public int height(BinNode<K> bn){
		if (bn == null){return 0;}
		else{
			return 1 + Math.max(height(bn.getLeft()), height(bn.getRight()));
			}
		}
	
	
	public boolean balanced(BinNode<K> top){
		System.out.println("Max height of left side of tree: " + height(top.getLeft()));
		System.out.println("Max height of right side of tree: " + height(top.getRight()));
		System.out.println("\nBalanced:");
		if (Math.abs(height(top.getLeft()) - height(top.getRight()) ) <= 1){
			return true;
		}
		else return false;
	}
